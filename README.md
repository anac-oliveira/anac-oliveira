<div align="center">

<!-- ═══════════════════════════════════════════════════════════════ -->

<!--                         SYSTEM BOOT                            -->

<!-- ═══════════════════════════════════════════════════════════════ -->

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0D1117,50:101820,100:00E5FF&height=220&section=header&text=ANA%20CLARA%20OLIVEIRA%20ALVES&fontSize=32&fontColor=00E5FF&fontAlignY=38&desc=FULL%20STACK%20%2F%20WEB%20DEVELOPER&descAlignY=60&descSize=18&animation=fadeIn"/>

<br>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=18&duration=2500&pause=700&color=00E5FF&center=true&vCenter=true&width=850&lines=%3E+INITIALIZING+SYSTEM...;%3E+WELCOME%2C+I'M+ANA+CLARA;%3E+FULL+STACK+%2F+WEB+DEVELOPER;%3E+CODE+FULLSTACK+%7C+SENAI+CYBER+E+IA;%3E+JAVASCRIPT+%7C+REACT+%7C+APIs+%7C+SQL+SERVER;%3E+BUILDING+IDEAS+INTO+REAL+APPLICATIONS..."/>

<br>

<img src="https://komarev.com/ghpvc/?username=anac-oliveira&style=for-the-badge&color=00E5FF&label=SYSTEM%20VISITS"/>

<a href="https://github.com/anac-oliveira">
<img src="https://img.shields.io/github/followers/anac-oliveira?style=for-the-badge&logo=github&logoColor=white&color=0D1117&labelColor=00E5FF&label=FOLLOWERS"/>
</a>

</div>

---

## `> SYSTEM.INIT`

```text
╔══════════════════════════════════════════════════════════════╗
║                    ARTIFICIAL INTELLIGENCE                  ║
║                       INTERFACE ONLINE                       ║
╠══════════════════════════════════════════════════════════════╣
║                                                              ║
║  USER       : ANA CLARA OLIVEIRA ALVES                      ║
║  HANDLE     : @anac-oliveira                                ║
║  ROLE       : FULL STACK / WEB DEVELOPER                    ║
║  EDUCATION  : CODE FULLSTACK — SENAI CYBER E IA             ║
║  LOCATION   : SÃO CAETANO DO SUL                            ║
║  STATUS     : ONLINE                                        ║
║                                                              ║
║  SYSTEM     : JAVASCRIPT • REACT • APIs • SQL SERVER        ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
```

---

# `> ABOUT_ME.exe`

<div align="center">

### 👩‍💻 Ana Clara Oliveira Alves

**Web / Full Stack Developer**

</div>

Sou **Ana Clara Oliveira Alves**, desenvolvedora apaixonada por tecnologia, desenvolvimento web e criação de aplicações.

🎓 **Sou formada em Code FullStack pelo SENAI Cyber e IA de São Caetano do Sul.**

Tenho conhecimentos em **JavaScript, HTML5, CSS3, React, APIs e SQL Server**, com foco no desenvolvimento de aplicações web, interfaces, integração entre sistemas e bancos de dados.

Gosto de transformar ideias em projetos práticos, aprender novas tecnologias e evoluir constantemente como desenvolvedora.

Meu objetivo é continuar construindo aplicações cada vez mais completas, unindo **Front-End + Back-End + Banco de Dados**.

```javascript
const ana = {

    identity: {
        name: "Ana Clara Oliveira Alves",
        username: "anac-oliveira",
        role: "Full Stack / Web Developer"
    },

    education: {
        course: "Code FullStack",
        institution: "SENAI Cyber e IA",
        location: "São Caetano do Sul",
        status: "Completed"
    },

    stack: [
        "JavaScript",
        "React",
        "HTML5",
        "CSS3",
        "APIs",
        "SQL Server"
    ],

    focus: [
        "Front-End",
        "Back-End",
        "Full Stack",
        "Web Development"
    ],

    mindset: "Learn → Build → Improve → Repeat"
};
```

---

# `> EDUCATION`

<div align="center">

<img src="https://img.shields.io/badge/🎓%20CODE%20FULLSTACK-00E5FF?style=for-the-badge&logoColor=0D1117"/>

### **Code FullStack**

**SENAI — Cyber e IA**

**São Caetano do Sul**

<img src="https://img.shields.io/badge/STATUS-COMPLETED-00E5FF?style=flat-square&labelColor=0D1117"/>

</div>

> Formação voltada ao desenvolvimento Full Stack, proporcionando conhecimentos práticos em programação, desenvolvimento web, APIs, banco de dados e construção de aplicações.

---

# `> TECH_STACK`

<div align="center">

### `// FRONT-END`

<img src="https://skillicons.dev/icons?i=js,html,css,react&theme=dark"/>

<br><br>

### `// BACK-END & APIs`

<img src="https://skillicons.dev/icons?i=cs,dotnet&theme=dark"/>

<br><br>

### `// DATABASE`

<img src="https://skillicons.dev/icons?i=sqlserver&theme=dark"/>

</div>

<br>

<table align="center">
<tr>
<td align="center" width="180">

**JavaScript**

<br>

`Logic • DOM • Web`

</td>

<td align="center" width="180">

**React**

<br>

`Components • UI`

</td>

<td align="center" width="180">

**HTML / CSS**

<br>

`Structure • Design`

</td>

<td align="center" width="180">

**SQL Server**

<br>

`Database • SQL`

</td>
</tr>
</table>

---

# `> TOOLS`

<div align="center">

<img src="https://skillicons.dev/icons?i=git,github,vscode,postman,visualstudio&theme=dark"/>

<br><br>

<img src="https://img.shields.io/badge/Git-0D1117?style=for-the-badge&logo=git&logoColor=00E5FF"/>
<img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=00E5FF"/>
<img src="https://img.shields.io/badge/VS%20Code-0D1117?style=for-the-badge&logo=visualstudiocode&logoColor=00E5FF"/>
<img src="https://img.shields.io/badge/Postman-0D1117?style=for-the-badge&logo=postman&logoColor=00E5FF"/>

</div>

---

# `> PROJECTS`

<div align="center">

<a href="https://github.com/anac-oliveira/BurguerStation.API">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=anac-oliveira&repo=BurguerStation.API&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00E5FF&icon_color=00E5FF&text_color=C9D1D9"/>
</a>

<a href="https://github.com/anac-oliveira/CodeBolosJacquin.API">
<img src="https://github-readme-stats.vercel.app/api/pin/?username=anac-oliveira&repo=CodeBolosJacquin.API&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00E5FF&icon_color=00E5FF&text_color=C9D1D9"/>
</a>

</div>

<br>

### 🍔 `01 // BURGUERSTATION.API`

Projeto de API relacionado a uma aplicação de hamburgueria, trabalhando com recursos de Back-End, banco de dados e gerenciamento de informações.

**Conceitos trabalhados:**

`API` `Back-End` `Database` `Products` `Categories` `Comments`

<a href="https://github.com/anac-oliveira/BurguerStation.API">
<img src="https://img.shields.io/badge/VIEW_PROJECT-00E5FF?style=for-the-badge&logo=github&logoColor=0D1117"/>
</a>

---

### 🍰 `02 // CODEBOLOSJACQUIN.API`

Projeto de API desenvolvido para trabalhar conceitos de desenvolvimento Back-End, integração e persistência de dados.

**Conceitos trabalhados:**

`API` `Back-End` `Database` `Integration`

<a href="https://github.com/anac-oliveira/CodeBolosJacquin.API">
<img src="https://img.shields.io/badge/VIEW_PROJECT-00E5FF?style=for-the-badge&logo=github&logoColor=0D1117"/>
</a>

---

### 🏠 `03 // LAARDECOR`

Projeto desenvolvido dentro da trajetória de estudos e construção de aplicações da Ana Clara.

O projeto faz parte do portfólio apresentado no GitHub.

<a href="https://github.com/anac-oliveira">
<img src="https://img.shields.io/badge/VIEW_ON_GITHUB-00E5FF?style=for-the-badge&logo=github&logoColor=0D1117"/>
</a>

---

# `> CURRENT_MISSION`

```text
┌──[ANA@GITHUB]─[~/mission]
│
├── $ system.current_goals
│
├── [✓] Formação Code FullStack
├── [✓] Desenvolvimento de aplicações web
├── [✓] Desenvolvimento de APIs
├── [✓] SQL Server & Banco de Dados
├── [✓] Desenvolvimento Front-End
│
├── [→] Evoluir como Full Stack Developer
├── [→] Aprofundar conhecimentos em React
├── [→] Criar aplicações mais completas
├── [→] Expandir conhecimentos em Back-End
└── [→] Transformar conhecimento em soluções reais
```

---

# `> GITHUB_ANALYTICS`

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=anac-oliveira&show_icons=true&hide_border=true&bg_color=0D1117&title_color=00E5FF&icon_color=00E5FF&text_color=C9D1D9&rank_icon=github&include_all_commits=true&count_private=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=anac-oliveira&layout=compact&hide_border=true&bg_color=0D1117&title_color=00E5FF&text_color=C9D1D9&langs_count=8"/>

</div>

<br>

<div align="center">

<img src="https://streak-stats.demolab.com?user=anac-oliveira&theme=tokyonight&hide_border=true&background=0D1117&ring=00E5FF&fire=00E5FF&currStreakLabel=00E5FF&sideLabels=C9D1D9&dates=8B949E" width="75%"/>

</div>

---

# `> CONTRIBUTION_ACTIVITY`

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=anac-oliveira&bg_color=0D1117&color=00E5FF&line=00E5FF&point=FFFFFF&area=true&hide_border=true&custom_title=ANA%20CLARA%20-%20CONTRIBUTION%20MATRIX" width="100%"/>

</div>

---

# `> ACHIEVEMENTS`

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=anac-oliveira&theme=algolia&no-frame=true&no-bg=true&margin-w=8&row=1&column=6" width="100%"/>

</div>

---

# `> SNAKE_PROTOCOL`

<div align="center">

<img src="https://raw.githubusercontent.com/anac-oliveira/anac-oliveira/output/github-contribution-grid-snake-dark.svg" alt="Snake Contribution Animation" width="100%"/>

<br>

```text
[ SYSTEM ]
Contribution matrix detected...

[ SNAKE ]
████████████████████████████████████████

[ STATUS ]
CONTRIBUTIONS → ANALYZED
SYSTEM → ONLINE
```

</div>

---

# `> CONNECT_WITH_ME`

<div align="center">

<a href="https://github.com/anac-oliveira">
<img src="https://img.shields.io/badge/GITHUB-0D1117?style=for-the-badge&logo=github&logoColor=00E5FF"/>
</a>

</div>

<br>

<div align="center">

```text
╔══════════════════════════════════════════════════════════╗
║                                                          ║
║                 CONNECTION ESTABLISHED                  ║
║                                                          ║
║              github.com/anac-oliveira                   ║
║                                                          ║
║        OPEN TO LEARNING • BUILDING • EVOLVING            ║
║                                                          ║
╚══════════════════════════════════════════════════════════╝
```

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00E5FF,50:101820,100:0D1117&height=130&section=footer&animation=fadeIn"/>

### `SYSTEM STATUS: ONLINE`

**Learn → Build → Improve → Repeat**

<br>

`© Ana Clara Oliveira Alves • @anac-oliveira`

</div>
